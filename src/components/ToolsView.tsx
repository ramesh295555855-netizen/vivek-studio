      ...
      <div className="max-w-7xl mx-auto space-y-12">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
          <div className="space-y-3">
            ...
            <h2 className="text-4xl font-black text-white uppercase italic tracking-tighter leading-none">AI TOOLS <br/><span className="text-cyan-400">SHOWCASE</span></h2>
          </div>
          <div className="relative group max-w-xs w-full">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-3 h-3 text-gray-500" />
            <input type="text" placeholder="Search tools..." className="w-full bg-white/5 border border-white/10 rounded-xl pl-10 pr-4 py-3 text-[10px] font-black text-white focus:outline-none focus:border-cyan-400/50" />
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {tools.map((tool, i) => (
            <motion.div
              ...
              className="p-8 rounded-[40px] bg-white/5 border border-white/5 hover:bg-white/10 hover:border-cyan-400/30 transition-all group relative overflow-hidden"
            >
              <div className="absolute top-0 right-0 w-24 h-24 bg-gradient-to-br from-cyan-400/5 to-purple-400/5 blur-3xl" />
              <div className="relative z-10 space-y-4">
                <div className={`w-12 h-12 rounded-xl ${tool.bg} flex items-center justify-center transition-transform group-hover:scale-110 group-hover:rotate-6`}>
                  <tool.icon className={`w-6 h-6 ${tool.color}`} />
                </div>
                <div className="space-y-1">
                  <span className="text-[9px] font-black uppercase tracking-[0.3em] text-gray-600">{tool.category}</span>
                  <h3 className="text-xl font-black text-white tracking-tight">{tool.title}</h3>
                  <p className="text-gray-500 text-xs font-medium leading-relaxed">{tool.desc}</p>
                </div>
        ...
        <div className="mt-12 p-10 rounded-[48px] bg-gradient-to-br from-cyan-900/20 via-purple-900/20 to-black border border-white/10 relative overflow-hidden group">
          ...
          <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            <div className="space-y-4">
              <h3 className="text-3xl font-black text-white tracking-tighter leading-none uppercase italic">Build Your Own <br/><span className="text-cyan-400">AI Agent</span></h3>
              <p className="text-gray-400 text-sm font-medium leading-relaxed">Leverage Maa Santoshi Studio's proprietary neural engine to create custom agents tailored to your business logic. A next-gen agentic builder platform.</p>
              <button className="px-8 py-4 rounded-xl bg-white text-black font-black text-[10px] uppercase tracking-widest hover:bg-cyan-400 transition-colors">Start Building (Beta)</button>
            </div>
